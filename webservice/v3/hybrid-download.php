...
// Prepare a temp file
$temp_file = tempnam('/tmp/', 'mpgsplice_');
// Check for faillure
if ($temp_file === FALSE) {
...
$descriptorspec = array(
   ...
   1 => array("file", $temp_file, "w"),	// stdout is the file the child will write to
...
$proc_stat = proc_get_status($process);
while ( $proc_stat['running'] === TRUE ) {
	usleep(200000); // Wait 2/10 of a second
	$proc_stat = proc_get_status($process);
}
